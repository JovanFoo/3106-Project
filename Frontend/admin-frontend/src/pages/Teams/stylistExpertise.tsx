...
export default function Expertise({ stylist }: Props) {
    ...
    useEffect(() => {
        const fetchStylistExpertise = async () => {
            try {
                const res = await axios.get(`${api_address}/api/stylists/${stylist._id}`, config);

                const stylistExpertise = res.data.expertises || [];

                // If populated: set directly. If only IDs, filter from allExpertise.
                if (stylistExpertise.length && typeof stylistExpertise[0] === "object") {
                    ...
                    {expertiseList.length === 0 ? (
                        ...
                        <div className="flex flex-wrap gap-3">
                            {expertiseList.map((item) => (
                                <div key={item._id} className="px-4 py-2 rounded-full bg-blue-600 text-white text-sm font-medium shadow-md">
                                    {item.name}
                                </div>
                            ))}
                        </div>
