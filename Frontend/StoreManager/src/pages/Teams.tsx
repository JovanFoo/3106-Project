...
export default function Teams() {
        ...
        const fetchData = async () => {
            try {
                ...
                const filteredStylists = stylistList.filter(
                    (stylist: TeamMember) => !teamList.some((member: TeamMember) => member._id.toString() === stylist._id.toString())
                );
    ...
    const handleAddStylist = async () => {
        try {
            const selectedStylist = availableStylists.find(s => s._id === selectedStylistId);
            if (!selectedStylist) return;

            const res = await axios.post(`${api_address}/api/teams`, { stylistId: selectedStylistId }, config);
            ...
            <div className="flex-1 p-5">
                ...
                <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">My Team</h3>
                        <Button size="sm" variant="primary" onClick={openModal}>Add Team Member +</Button>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {teamMembers.map((member) => (
                            <div key={member._id} className="p-4 border rounded-lg shadow-sm flex flex-col items-center">
                                ...
                                {member.rating && <p className="text-yellow-500">⭐ {member.rating}</p>}
                                <Button
                                    size="sm"
                                    variant="primary"
                ...
                <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[600px] p-6">
                    <div className="flex flex-col px-2 overflow-y-auto">
                        <h4 className="text-lg font-semibold text-gray-800 dark:text-white/90 mb-4">Add Existing Stylist</h4>
                ...
                {selectedStylist && (
                    <Modal isOpen={isViewModalOpen} onClose={closeViewModal} className="max-w-[900px] p-0">
                            ...
                            <div className="w-64 bg-gray-100 dark:bg-gray-800 p-4">
                                {["Profile", "Expertise", "Portfolio", "Testimonials"].map((tab) => (
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-2 rounded-md mb-2 ${activeTab === tab ? "bg-blue-600 text-white" : "text-gray-700 dark:text-white hover:bg-gray-200 dark:hover:bg-gray-700"
                                            }`}
                                    >
                                        {tab}
                                    </button>
