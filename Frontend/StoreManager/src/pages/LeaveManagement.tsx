...
import { format, addDays, differenceInDays, eachDayOfInterval, startOfWeek, endOfWeek } from 'date-fns';
...
const LeaveManagement: React.FC = () => {
  ...
  const [selectedStatus, setSelectedStatus] = useState<string[]>(['Pending', 'Approved']);
  const [selectedTypes, setSelectedTypes] = useState<string[]>(['Bereavement', 'Casual', 'Paid', 'Sick', 'Volunteer']);
            ...
            {Object.entries({
              ...
              <Chip
                ...
                sx={{
                  ...
                  '&:hover': {
                    bgcolor: selectedTypes.includes(type)
                      ? alpha(getLeaveTypeColor(type as LeaveRequest['type']), 0.8)
                      : 'grey.200'
                  }
                }}
              />
                          ...
                          <Grid item xs key={i}>
                            <Box sx={{ 
                              ...
                              {dayRequests.map(request => {
                                ...
                                const isMiddleDay = day > startDate && day < endDate;

                                return (
                                  <Tooltip
                                    key={request._id}
                                    title={`${request.type} (${request.status}): ${format(startDate, 'MMM dd')} - ${format(endDate, 'MMM dd')}`}
                                  >
                                    <Box 
                                      sx={{ 
                                        position: 'absolute',
