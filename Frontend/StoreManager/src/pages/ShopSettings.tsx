...
export default function ShopSettings() {
    ...
    useEffect(() => {
        ...
        const fetchShop = async () => {
            try {
                setIsLoading(true);
                const res: AxiosResponse = await axios.get(`${api_address}/api/branches/shops`, config);
    ...
    const handleSave = async () => {
        try {
            ...
            await axios.put(`${api_address}/api/branches/${updated._id}`, updated, config);
            setShopData(updated);
            closeModal();
            setVariant("success");
            setTitle("Success");
    ...
    return (
        <div className="flex min-h-screen">
            <div className="flex-1 p-5">
                <PageMeta title="Shop Settings" description="Stylist's assigned shop details" />
                ...
                <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
                    ...
                    {shopData ? (
                        <div className="relative p-4 border rounded-md mb-4">
                            ...
                            <p><strong>Phone:</strong> {shopData.phoneNumber}</p>
                            <p><strong>WeekdayOpeningTime:</strong> {shopData.weekdayOpeningTime}</p>
                            <p><strong>WeekdayClosingTime:</strong> {shopData.weekdayClosingTime}</p>
                            <p><strong>WeekendOpeningTime:</strong> {shopData.weekendOpeningTime}</p>
                            <p><strong>WeekendClosingTime:</strong> {shopData.weekendClosingTime}</p>
                            <p><strong>Public Holiday Opening Time:</strong> {shopData.holidayOpeningTime}</p>
                            <p><strong>Public Holiday Closing Time:</strong> {shopData.holidayClosingTime}</p>
                            <div className="absolute bottom-2 right-2">
                                <Button size="sm" variant="primary" onClick={handleOpenModal}>Edit</Button>
                            </div>
                        </div>
                    ) : (
                ...
                <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[600px] p-6">
                    {shopData && (
                        <div className="flex flex-col px-2 overflow-y-auto">
                            <h4 className="text-lg font-semibold text-gray-800 dark:text-white/90 mb-4">Edit Shop</h4>
                            <div className="grid grid-cols-1 gap-y-4">
                                ...
                                {["weekday", "weekend", "holiday"].map((key) => (
                                    <div key={key}>
                                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-400 capitalize">{key} hours</label>
                                        <div className="flex gap-2">
                                            <select
                                                value={shopData[`${key}OpeningTime` as keyof Shop] as string}
                                                onChange={(e) => setShopData({ ...shopData, [`${key}OpeningTime`]: e.target.value })}
                                                className="w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600"
                                            >
                                            ...
                                            <select
                                                value={shopData[`${key}ClosingTime` as keyof Shop] as string}
                                                onChange={(e) => setShopData({ ...shopData, [`${key}ClosingTime`]: e.target.value })}
                                                className="w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600"
                                            >
                                                {openingHours.map((hour) => (
                                                    <option key={hour} value={hour}>{hour}</option>
